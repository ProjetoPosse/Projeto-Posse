...
function buildMentorKpis(data, mentorados) {
  const items = data.monthlyItems || [];
  const checkins = data.dailyCheckins || [];
  const mentoradoMap = new Map((mentorados || []).map((item) => [item.id, item]));
  return (mentorados || []).map((mentorado) => {
    ...
    return {
      ...
      concurso: mentoradoMap.get(mentorado.id)?.concurso_id || null,
...
function getDefaultDemoData() {
  ...
  return {
    ...
    simulados: [
      { id: "demo-simulado-1", mentorado_id: mentorado1Id, concurso_id: concurso1Id, titulo: "Simulado 1 - Diagnostico TRF-3", data_aplicacao: "2026-05-23", acertos: 41, total_questoes: 62, observacoes: "Base para ajustar Penal, Processo Penal e RLM no meio do ciclo." },
      { id: "demo-simulado-3", mentorado_id: mentorado1Id, concurso_id: concurso1Id, titulo: "Simulado 3 - Consolidacao", data_aplicacao: "2026-06-20", acertos: 46, total_questoes: 62, observacoes: "Evolucao consistente em Tributario e Previdenciario; ainda reforcar topicos TRF-3." },
      { id: "demo-simulado-2", mentorado_id: mentorado2Id, concurso_id: concurso2Id, titulo: "Simulado Auditor", data_aplicacao: isoOffsetDay(-5), acertos: 28, total_questoes: 50, observacoes: "Precisa reforcar Controle Externo." }
    ],
    weekly: [
...
async function fetchMentoradoData() {
  ...
  const [
    ...
    client.from("simulados").select("id,titulo,data_aplicacao,acertos,total_questoes,observacoes").order("data_aplicacao", { ascending: false }),
...
async function fetchAdminData() {
  if (isDemoMode()) {
    ...
    return {
      ...
      simulados: deepClone(data.simulados),
  ...
  const [profilesRes, concursosRes, materiaisRes, monthlyRes, itemsRes, simuladosRes, dailyCheckinsRes] = await Promise.all([
    ...
    client.from("simulados").select("id,titulo,data_aplicacao,acertos,total_questoes,mentorado_id").order("created_at", { ascending: false }).limit(20),
...
async function renderAdminPage() {
  ...
  const mentorKpis = buildMentorKpis(data, data.mentorados);
